    ...
    await fetch('http://backend.test.ikoniks.de/api/website/contacts', {
      ...
      .then(res => {
        if (res.ok) {
          setNotification(pre => ({
            ...pre, show: true
            , message: t("your message sent successfully"), severity: 'success'
          }))
          setLoading(false)
          emptyFields()
          ...
          setNotification(pre => ({
            ...pre, show: true
            , message: t("your message was not send"), severity: 'error'
          }))
          setLoading(false)
        }
      })
    ...
    <MenuContextProvider>
      <SearchContextProvider>
        <Layout PageTitle="Contact Page">
          <HeaderOne />
          <PageBanner title={t("Contact Us")} name="Contact" />
          <ContactForm
            fields={fields}
...
export async function getStaticProps({ locale }) {
  ...
  return {
    props: { ...await serverSideTranslations(locale, ['common']) }, revalidate: 300,
  }
}